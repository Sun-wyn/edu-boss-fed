...
request.interceptors.request.use(function (config) {
  // 就可以在这里通过修改 config 配置信息来实现业务功能的统一处理
...
request.interceptors.response.use(function (response) {
  ...
  // 超出 2xx 状态码都执行这里
  // 如果是http错误状态码，错误处理写到这里
  if (error.response) { // 请求收到响应 但是状态码超出了 2xx 错误
